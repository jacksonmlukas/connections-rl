...
recovers 26 group slots, supervised fine-tuning 56, the step-50 GRPO policy
77, and the step-403 endpoint 4 --- against a chance floor of 1.4 slots for a
uniformly drawn valid partition. Step 50 improves on the untrained model by a
...
\emph{training} reward mean records none of this: it reaches the reward's ceiling
of 1.6 by step 125, with across-rollout variance zero on nearly every prompt
from then on --- the variance, not the mean, is the collapse's only in-sample
trace --- while the identical reward evaluated out of sample records the
...
GRPO is initialized from, and step 50 --- the two stages contribute
comparably ($+26$ to $+30$ supervised, $+21$ to $+26$ reinforcement learning;
the SFT arm decodes to 56 or 52 slots depending on serving session,
...
reward sits at 1.6, the maximum it can emit, with
...
in one serving session and 52 (0.321) in another, differing on 2 of 162
puzzles (Appendix~\ref{app:endpoints}), so the stage accounting is $26
\rightarrow 52\text{--}56 \rightarrow 77 \rightarrow 4$: SFT contributes
$+26$ to $+30$ and GRPO $+21$ to $+26$ at its peak --- comparable
contributions, and the data do not resolve which is larger. The SFT arm
shares no serving session with Table~\ref{tab:arc}'s, so this is the one
...
0.2525 --- the highest held-out reward we measured: the reward peak, like
the groups peak, is a plateau. The training reward meanwhile reaches its ceiling by
...
$+0.67$ to $+0.69$ between merged weight-delta directions, against
$\mathrm{E}[\lvert\cos\rvert] = \sqrt{2/(\pi d)} \approx 10^{-5}$ for a
random pair at $d \approx 6.5 \times 10^{9}$. Different random draws found
the same attractor. Best-of-16 sampling does
...
structure begins to be bought and semantics begins to be sold. It is a broadly
monotone structural climb --- one reversal, at step 100 --- with a semantic
hump on top of it, and only the hump has a location. One instrumentation gap
...
reward that does not depend on the input. An argument for plausibility, not
...
recovers 4. Two qualifications keep this a path description, not a scale.
...
seed 0). The training reward (gray; TRL's 5-step interval averages from the
run history)
first touches the reward function's ceiling of 1.6 at step 125 and averages
1.5814 from then on, with occasional excursions and no trend.
...
125 and averages 1.5814 over the logged points from then on, with occasional
excursions --- 15 of 57 fall more than 0.02 below the ceiling, the deepest to
1.4300 at step 155 --- and no trend. What the figure cannot show is TRL's own
...
the optimizer receives no learning signal on nearly every prompt. Read as a
diagnostic: the in-sample mean records none of the reversal; the in-sample
\emph{variance} records most of it, bracketing the collapse between logged
steps 100 and 150 with no held-out data. What the variance cannot supply is
a sign --- zero spread is what mastery and degeneracy both look like --- so
held-out evaluation still decides which. What
...
normalizes \texttt{False} to \texttt{"none"}. Adversarial reviewers of an
earlier draft independently identified it as a confound:
...
policy below the untrained Instruct model despite its near-perfect output
format. That ordering rules out the obvious confound: the final policy is the
...
can still distinguish. The training reward cannot record the reversal; a
blind judge can. Judges do err in
...
(0.200 to 0.433). The nine intervals are simultaneous and uncorrected; all
exclude zero by margins no standard multiplicity correction would close.
...
trainer's own variance diagnostics, which bracket the collapse in sample
though they cannot sign it (\S\ref{sec:mechanism}).
...
selected as the argmax of a nine-point validation grid on which step 100
scores 0.0875 to step 50's 0.0950 at $n=100$ --- inside one standard
error. The grid preceded every test evaluation of the selected checkpoint
(the ablation's peak was likewise frozen from validation first), so this is
ordinary validation-based selection, and the step-100 test evaluation
(\S\ref{sec:arc}) shows it rests on a plateau, not a lucky column.
...
independently satisfiable --- a property of its design, not its fit --- and
determinism is the exact property cited when practitioners argue that
verifiable rewards escape this failure mode.
...
instruction-tuned checkpoint, with RL initialized from a supervised
checkpoint above it); here RLVR ends below \emph{both} of its own
antecedents on the construct.
...
while the reward's cheap component still improves. \citet{drgrpo} critique group-level advantage normalization; our
...
changes in learned reward and external judge scores, reporting that
row-level diagnostics surface failures checkpoint averages hide. Two
things differ here: our reward is rule-based, so the gap we report is not
reward-model misgeneralization but a gap between a correct rule and the
construct it stands in for; and our reversal is bracketed rather than
flagged --- the collapsed policy scores below the untrained Instruct model
on held-out data, dating the turn instead of detecting it.
...
unusually cleanly. The checkpoint trajectory is 7B seed 0 only (the only
run Hub-synced during training) --- so the scale-dependence
...
1.5B (\S\ref{sec:scale}, which also notes the epoch asymmetry).
...
distrust single numbers elsewhere in this paper. The peak's height and
location may both move across seeds --- unmeasured --- though the step-100
evaluation shows it is not a knife edge.

We report a single $\beta$ alongside one that froze the policy, and no
intermediate run; the two configurations differ in learning rate as well as
$\beta$, and $\beta=0.001$ contributed at most 0.047 reward units in the
reported run. Entropy regularization and a larger KL penalty are untested
...
on; whether SFT-stage memorization explains that is unmeasured --- neither
SFT nor the final policy has been scored on training puzzles --- and open.
