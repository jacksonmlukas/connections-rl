...
recovers 26 group slots, supervised fine-tuning 56 (52 in a second
serving), the step-50 GRPO policy
...
\emph{training} reward reads the collapse as success: its steepest climb,
0.99 to its 1.6 ceiling, falls between steps 100 and 125 --- inside the
...
GRPO is initialized from, and step 50 --- the supervised stage contributes
at least as much as the reinforcement-learning stage ($+26$ to $+30$ against
$+21$ to $+25$; the SFT arm decodes to 56 or 52 slots depending on serving
session, \S\ref{sec:arc}). Over the following
...
$+26$ to $+30$ and GRPO $+21$ to $+25$ at its peak --- the supervised share
is at least as large under both servings, decisively at 56, a near-tie at
52. The SFT arm
shares no serving session with Table~\ref{tab:arc}'s, so this is the one
contrast we cannot pair; we report both servings rather than either alone.
...
never recovers. The structural score (fraction of valid partitions, not
...
input-independent component; whether what survives on \emph{training}
...
held-out data by a wide margin. The run spent 47.05: 77 slots at 5.7\% of
...
steepest climb (0.9931 to 1.6000, 41.8\% of its observed range) and the
...
prompts that still carry signal, but we have not measured that chain.
...
failure is invisible in the sum, obvious in the decomposition, and the
...
\citet{agarwal2021} motivate our
...
on; whether SFT-stage memorization explains that is unmeasured and open ---
neither SFT nor the final policy has been scored on training puzzles.
