...
written to return at every point in policy space, and the argument that this
...
untrained base model, 26 recovered group slots of 648 to 77, clearing the
supervised checkpoint the run is initialized from. Over the following 353
steps the same run falls to 4 slots, below the untrained base model, while
the invalid-output rate falls by an order of magnitude and the training
reward sits at 1.6 --- the maximum the reward function can emit --- with
zero variance across the $K=8$ rollouts on nearly every prompt. A
...
leakage-controlled case study in which GRPO first triples and then destroys
the target construct, ending below the untrained base model across three
seeds (\S\ref{sec:arc}); a checkpoint-level account placing the semantic
peak at 5.7\% of the run's KL displacement, with entropy collapse as the
mechanism and the training reward at its ceiling --- zero group-relative
...
four adversarial reviewers of an earlier draft independently named
(\S\ref{sec:ablation}); a blind LLM-judge evaluation showing the reversal
is legible from outside the reward (\S\ref{sec:judge}); and a 1.5B scale
contrast where the same recipe optimizes only what it can verify
(\S\ref{sec:scale}).
...
base at step 50, $-0.136$ $[-0.191, -0.080]$ at step 403, both intervals
excluding zero, with the supervised checkpoint the run was initialized from
at 56 slots and 0.346 (separate session; no paired interval attached).
Means in Table~\ref{tab:arc} are exact ratios of the count column, and we
report both because the paper's multiplicative language describes the
difference between 26, 77 and 4 slots of 648. Two details locate the
mechanism. Held-out mean reward peaks at step 50 at 0.246 and ends at
0.125, below base's 0.165, while the training reward reaches its ceiling by
...
are single-seed and we read them as illustrative. (Per-arm values for the
three-seed session are tabulated in Appendix~\ref{app:endpoints}.)
...
\paragraph{Entropy is the mechanism.} Policy entropy falls 13.4-fold
