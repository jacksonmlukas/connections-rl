...
| Item | File(s) | Status |
|---|---|---|
| B1 scale_rewards=False run | `taskB1_grpo-7b-noscale.yaml`, `taskB1_grpo_scale_rewards.patch` | prepared — **requires the grpo.py patch first** (config alone is silently ignored). Patch is three insertions: conditional kwarg, signature guard, and a **read-back off the constructed GRPOConfig** that aborts on mismatch and prints resolved value + TRL version (R3a). The handoff's dry check is notebook cell 2b; do not launch until it passes |
...
| B3 / R1+R2 W&B exports | `b3_wandb_export.py` | prepared — owner runs locally with W&B login; writes `data/wandb_train_reward.csv` (raw scan_history rows) and `data/step_count_1p5b.json` (settles 403-vs-806 off the final logged `_step`, never the config) |
| R-handoff path mapping | `r_map_deliverables.py` | prepared — maps session outputs to the exact `data/` paths the brief names, computes recovered-group counts /648, and gates the R6 generation files on exact reproduction of 26/77/4 |
...
| C2 LLM-judge row | harness `capture_generations` flag, `c2_capture_generations.py`, `c2_judge_openrouter.py` | prepared — **generations are not stored anywhere in the repo** (records keep scores only). R6: the eval harness now takes an opt-in `capture_generations: true` config key (default off, behavior unchanged) writing `<arm>/generations.jsonl` in the brief's exact schema; `taskB2_eval_test.yaml` sets it |
